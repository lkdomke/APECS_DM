...
```{r data}
seine <- read.csv("../Data/Beach_seine_eelgrass_2018_RAW.csv") # read in WR seines
seine_trial <- read.csv("../Data/FishSeine_RawData_2018_Trial.csv") # read in trial exp data
sp_names <- read.csv(url("https://knb.ecoinformatics.org/knb/d1/mn/v2/object/urn%3Auuid%3Affa06540-6689-41d8-b8ac-052a9f568785"))
...
fishLW <- read.csv(url("https://knb.ecoinformatics.org/knb/d1/mn/v2/object/urn%3Auuid%3Acd2ff373-ba90-449d-85a2-4e1fb174a926")) 
```
...
seine_trial <- seine_trial %>%
  ...
  dplyr::rename(sex = Sex, notes = Notes.)

# fix a few rows where the date is incorrect
seine_trial[1132:1138,"date"] <- "6/30/2018"
# removes rows that excel generated with all NAs
seine_trial <- seine_trial[-(1140:1277),]
...
seine_trial_sub <- seine_trial_sub %>%
  ...
  dplyr::select(site, date, YYYYMMDD, start_time, end_time, slope, tide_height, tide_time, 
                species_common, species_scientific, sp_code, fork_length, unmeasured, sex, notes, 
                taxon)
```
...
```{r}
levels(as.factor(seine_all$species_common)) 
# 41 different species, need to remove non-fish species

# remove non-fish species
fish <- seine_all %>% 
  filter(taxon == "Vertebrata" | taxon == "fish")

levels(as.factor(fish$species_common)) # only fish!
levels(as.factor(fish$site)) # still have all the sites! 
```
...
fish.site <- unique(fish[,c("site", "date", "YYYYMMDD", "start_time", "end_time", "slope", "tide_height", "tide_time")])
