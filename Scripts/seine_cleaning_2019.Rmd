...
nonmatch1 # There a few more fish w/o length-weight relationship will be dealt with below. 
```
...
rock <- data.frame(sp_code = "UNROCK", fishLW %>% 
                     ...
                     summarise(a = mean(a_cm.g),
                               b = mean(b_cm.g)))
## Master a b lsit ##
LW.master <- rbind.data.frame(LW, scul, flat, grns, myoxo, gunn, arte, sal, rock)
...
unique(anti_join(fish.all, LW.master, by = "sp_code")$sp_code)
```
...
```{r}
fish.all.m <- fish.all.m %>% 
  left_join(site_meta, by = c("site" = "site_2019")) %>% # this needs to be changed for e/a YEAR. 
