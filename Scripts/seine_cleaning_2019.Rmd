...
q <- bind_rows(q, m_shiner, m_stick)
```
...
unique(fish$sp_code) # 55 levels of fish species in the original dataframe, correct! 7

# count total number of fish in original dataframe
fish$abundance <- as.numeric(ifelse(is.na(fish$fork_length), paste(fish$unmeasured), 1)) 

fish %>%
  dplyr::summarise(counts = sum(abundance)) # 22762
...
fish.all %>%
  dplyr::summarise(counts = sum(abundance)) # 25977
```
