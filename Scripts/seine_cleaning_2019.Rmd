...
Shiner perch first: based on distributions of known fork length we set a cut off at 100 mm 
```{r}
# Need for data frames
...
Subset the rest of the data into m and um (MAKE SURE TO REMOVE THE SPECIES YOU ALREADY DID)
```{r}
fish.m <- fish %>%
  filter(fork_length != "NA")
...
x <- fish.um %>%
  ...
  anti_join(fish.m, by = c("site", "sp_code")) # a couple of fish that will be dealt with later in the script. (Will combine with other UN species)
...
# Export
```{r}
write.csv(fish.all.m, "../Data/Fish_mass_full_2019_derived.csv", row.names = FALSE)
```
